...
import type { Post, SMInsight, DateRange, UTMLink } from '@/types/smdashboard';
import * as api from '@/lib/smdashboard/api';

import SMLoginGate from '@/components/smdashboard/SMLoginGate';
...
export default function LoginPage() {
  return (
    <>
      ...
      <SMLoginGate>
        {({ user, org, logout }) => (
          <Dashboard user={user} org={org} logout={logout} />
        )}
      </SMLoginGate>
    </>
  );
...
interface DashboardProps {
  user: { id: string; name: string; username: string; role: 'org_owner' | 'creator'; org_id: string; email: string | null };
  org: { id: string; name: string; slug: string; shop_domain: string };
  logout: () => void;
}

function Dashboard({ user, org, logout }: DashboardProps) {
  ...
  // Fetch saved links from real API
  ...
  // TODO: Replace with real API data when backend wiring is complete (Task 12)
  ...
  return (
    <>
      <UserLayout storeName={org.name} onLogout={logout}>
        {loading ? (
          <UserLoadingSkeleton />
        ) : (
          <motion.div
          ...
          >
            ...
            <div className="rounded-lg border border-zilla-neon/20 bg-zilla-neon/5 px-4 py-3 flex items-center gap-3">
              ...
              <p className="text-sm text-gray-300">
                Welcome back, <span className="text-zilla-neon font-medium">{user.name}</span>
                {' '}&middot; {user.role === 'org_owner' ? org.name : `@${user.username}`}
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  ...
                  >
                    Saved Links{savedLinks.length > 0 ? ` (${savedLinks.length})` : ''}
                  </button>
                  {user.role === 'org_owner' && (
                    <CreatorSelector
      ...
      {/* Saved Links Sidebar (real API data) */}
      <SavedLinksSidebar
        open={showSidebar}
